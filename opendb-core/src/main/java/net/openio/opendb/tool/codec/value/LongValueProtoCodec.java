...
public class LongValueProtoCodec implements ValueProtoCodec {
  ...
  private void encodeValue(ByteBuf buf, Value value) {
    ...
    Codec.encodeVarInt64(buf, Codec.encodeZigzag64((long) value.getValue()));
  }

  private void decodeValue(ByteBuf buf, Value value) {
    value.setValue(Codec.decodeZigzag64(Codec.decodeVarInt64(buf)));
  }
  ...
  public int getByteSize(Value value) {
    ...
    length += Codec
      .computeVarInt64Size(Codec
        .encodeZigzag64((long) value.getValue()));
