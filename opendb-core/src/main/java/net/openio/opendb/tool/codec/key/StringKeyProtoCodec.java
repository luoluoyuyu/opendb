...
public class StringKeyProtoCodec implements KeyProtoCodec {
  ...
  private void encodeValue(ByteBuf buf, Key key) {
    ...
    Codec.encodeString(buf, (String) key.getKey());
  }

  private void decodeValue(ByteBuf buf, Key key) {
    key.setKey(Codec.decodeString(buf, Codec.decodeVarInt32(buf)));
  }
  ...
  public int getByteSize(Key key) {
    int length = ByteBufUtil.utf8Bytes((String) key.getKey());
