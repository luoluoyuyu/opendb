...
public class DoubleKeyProtoCodec implements KeyProtoCodec {
  ...
  private void encodeValue(ByteBuf buf, Key key) {
    ...
    Codec.encodeDouble(buf, (double) key.getKey());
  }

  private void decodeValue(ByteBuf buf, Key key) {
    key.setKey(Codec.decodeDouble(buf));
  }
